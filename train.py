...
#     # Set the device to this process's rank.
#     torch.cuda.set_device(rank)
#     device = torch.device('cuda:{}'.format(rank))

#     main(args, rank, device, world_size)
...
#     dataset = SHHA(data_root, transform, train, patch, flip)
...
#         shuffle=(dist_sampler is None),  # turn off shuffle if using DistributedSampler
#         sampler=dist_sampler,
#         num_workers=args.num_workers,
#         collate_fn=utils.collate_fn_crowd,  # your custom collate function
#         pin_memory=True)
...
def main(rank, args, num_gpus):
    ...
    # device = 
    # set the device to the rank of the current process
    device = torch.device('cuda:{}'.format(rank))
    ...
    # create the batch sampler for training
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        stat = train_one_epoch(model, criterion, data_loader_train, optimizer, rank, epoch, args.clip_max_norm)
